...
public class AuthController {
    ...
    // === 3. CẬP NHẬT HÀM LOGIN ĐỂ BẮT LỖI 401/403 ===
    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) { // Đổi kiểu trả về thành <?>
        ...
        try {
            ...
            // Nếu thành công, trả về OK (200)
            return ResponseEntity.ok(responseMap);

        } catch (RuntimeException e) {
            // --- BẮT LỖI TẠI ĐÂY ---
            // Bắt lỗi "Sai SĐT" (UsernameNotFoundException)
            // HOẶC lỗi "Sai mật khẩu" (RuntimeException từ UserService)

            System.err.println("!!! LỖI ĐĂNG NHẬP (BỊ BẮT TẠI AuthController): " + e.getMessage());

            // Trả về lỗi 401 (Unauthorized)
            // Frontend (DoctorLoginPage) sẽ bắt lỗi này và hiển thị
    ...
    public ResponseEntity<Map<String, String>> checkExist(
        ...
        if (phoneNumber != null && !phoneNumber.isEmpty() && userService.isPhoneNumberExists(phoneNumber)) {
            response.put("error", "Số điện thoại đã tồn tại");
            return ResponseEntity.badRequest().body(response);
        }

        if (email != null && !email.isEmpty() && userService.isEmailExists(email)) {
            response.put("error", "Email đã tồn tại");
            return ResponseEntity.badRequest().body(response);
        }
    ...
    // API lấy thông tin user hiện tại (cho Bệnh nhân, Bác sĩ, Admin)
    @GetMapping("/me")
    public ResponseEntity<User> getMyProfile() {
        User user = userService.getCurrentUser();
        return ResponseEntity.ok(user);
